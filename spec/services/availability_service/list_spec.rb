...
RSpec.describe AvailabilityService::List, type: :model do
  ...
  it 'returns 2 availabilities' do
    AvailabilityService::Create.new(user_id: dummy_id, from: from.change(hour: 11, min: 0)).execute
    AvailabilityService::Create.new(user_id: dummy_id, from: from.change(hour: 17, min: 0)).execute
