...
RSpec.describe AvailabilityService::Delete, type: :model do
  ...
  it 'returns success when deleting existing availability' do
    availability = Availability.create(user_id: dummy_id, from: from.change(day: 12, month: 10, year: 2020, hour: 11, min: 0))
  ...
  it 'returns error when deleting non-existent availability' do
    availability =  Availability.create(user_id: 'test_2', from: from.change(day: 12, month: 10, year: 2020, hour: 17, min: 0))
